  ...
  <div class="starter-template">
    ...
    <div class="col-md-12">
      ...
      if(!isset($alteracoes_raiz[0])){
        ...
        <br>
        <br>
        <h4>Alterações de periápice</h4>

        <?php
        if(!isset($alteracoes_periapice[0])){
          echo "Não há alterações de periápice";
        } else{
          foreach($alteracoes_periapice[0] as $key => $alt){
            $texto = "";
            if($key != "id" && $key != "id_exame" && $alt != ""){
              echo $dic[$key]. ": ";
              $alt = explode(",", $alt);
          ...
          foreach($dentes as $dent){
            if($exame[0]['denticao'] == "mista" || $exame[0]['denticao'] == "permanente" && $dent <= 48 || $exame[0]['denticao'] == "decidua" && $dent >= 51){
              ...
              if (in_array($dent, $dentes_aus)) { 
                ...
                $mv = "";
                $cmv = "";
                foreach($alteracoes_periapice[0] as $key => $alte){
                  if($key != "id" && $key != "id_exame" && $alte != ""){
                    $alt = explode(",", $alte);
                    foreach($alt as $al){
                      $al = explode("_", $al);
        ...
        <?php
        if(!isset($alteracoes_coa[0])){
          echo "Não há alterações de crista óssea alveolar";
        } else{
          foreach($alteracoes_coa[0] as $key => $alt){
            $texto = "";
            if($key != "id" && $key != "id_exame" && $alt != "" && $key != "perda_ossea_generalizada" && $key != "perda_ossea_generalizada2"){
              echo $dic[$key]. ": ";
              $alt = explode(",", $alt);
              foreach($alt as $al){
              ...
              echo trim($texto,", ").".";
              echo "<br><br class='notprint'>";
            } else if($key == "perda_ossea_generalizada"){
              $texto .= $dic[$key]. ": ".$alteracoes_coa[0]["perda_ossea_generalizada"]." ".$alteracoes_coa[0]["perda_ossea_generalizada2"];
              echo trim($texto,", ").".";
              echo "<br><br class='notprint'>";
            } 
          ...
          foreach($dentes as $dent){
            if($exame[0]['denticao'] == "mista" || $exame[0]['denticao'] == "permanente" && $dent <= 48 || $exame[0]['denticao'] == "decidua" && $dent >= 51){
              ...
              if (in_array($dent, $dentes_aus)) { 
                ...
                $o = "";
                $co = "";
                foreach($alteracoes_coa[0] as $key => $alte){
                  if($key != "id" && $key != "id_exame" && $alte != ""){
                    $alt = explode(",", $alte);
                    foreach($alt as $al){
                      $al = explode("_", $al);
